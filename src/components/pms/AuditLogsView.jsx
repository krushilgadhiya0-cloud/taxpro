...
export default function AuditLogsView({ onShowToast }) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      // Initial seed logs if completely empty
        ...
        const defaultSeeds = [
          {
            ...
            action: 'CALL_RECORDING',
            module: 'Communication & Calls',
            details: 'Encrypted Voice Call with CA Ananya Sharma (03:42 min). Audio recording archived in practice vault.',
            ip_address: '127.0.0.1 (Local Verified)',
            metadata: {
              callType: 'audio',
              duration: 222,
              durationFormatted: '03:42',
              contactName: 'CA Ananya Sharma (Senior Auditor)',
              fileSize: '342.5 KB',
              recordingName: 'TaxPro_VoiceCall_Ananya_Sharma_Audit.webm'
            },
          ...
          },
          {
            ...
            details: 'Printed Official Client Dossier & GST Compliance Statement for Acme Technologies Pvt Ltd.',
            ip_address: '127.0.0.1 (Local Verified)',
            metadata: { printType: 'Client Dossier' },
          ...
          },
          {
            ...
            module: 'Practice Financials',
            details: 'Recorded Client Fee Receipt of ₹45,000 via IMPS for Statutory Audit 2026.',
            ip_address: '127.0.0.1 (Local Verified)',
            metadata: { amount: 45000, mode: 'IMPS' },
          ...
          },
          {
            ...
            action: 'CALL_RECORDING',
            module: 'Communication & Calls',
            details: 'Video Conference with Rahul Verma (Tax Specialist) regarding MCA ROC Filings (05:18 min).',
            ip_address: '127.0.0.1 (Local Verified)',
            metadata: {
              callType: 'video',
              duration: 318,
              durationFormatted: '05:18',
              contactName: 'Rahul Verma (Tax Specialist)',
              fileSize: '488.0 KB',
              recordingName: 'TaxPro_VideoCall_Rahul_Verma_ROC.webm'
            },
            created_at: new Date(Date.now() - 1000 * 60 * 320).toISOString()
          }
        ];
        combined.push(...defaultSeeds);
        try {
          localStorage.setItem('taxpro_audit_logs', JSON.stringify(defaultSeeds));
        } catch (e) {}
      }
